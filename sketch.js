...
hoop.setCollider("circle",0,0,100);
 ...
 obstacle=createSprite(Math.round(random(50,500)),400,50,50);
...
if(rand>1){
 ...
 if(obstacle.isTouching(basketball)){
   basketball.velocityY = 0;
   obstacle.velocityY = 0;
   obstacle.velocityX = 0;
 }
 if(hits>=3){
   text("GAME OVER: YOU LOST!",300,300);
   hitAgain();
   obstacle.velocityX = 0;
   obstacle.velocityY = 0;
 }
...
function spawnObstacle(){
  if(frameCount%60==0){
    var obstacle=createSprite(Math.round(random(50,500)),400,50,50);
    obstacle.scale=0.2;
    obstacle.addImage(obstacleImg);
